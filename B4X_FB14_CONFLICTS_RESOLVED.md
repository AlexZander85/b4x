...
| 145 | No flag-day migration | L8 содержит общую migration проверку, но нет MON strangler/direct-apply reachability и общих cutover invariants | `RESOLVED` (FB-35) | Каноническая матрица `src/validation/no_flag_day.go` (§145): per-subsystem shadow/cutover registry с executable evidence; monitoring cutover доказывает reverse-reachability clean (`applyBatchResults` недостижим) + forward-reachability нового производственного path (ObservationBus/DiagnosticScheduler); seeded-реактивация legacy пути детектируется; shadow subjects — feature-gated compat surfaces (QUIC→TCP handoff, TCP reassembly, hard-gate alias migration); MON gate fail-closed при reactivation. ARCH-145 → PASS (B4X_FB18B_CROSSWALK.json, 07.08). |

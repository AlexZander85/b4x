...
28. **b4x-rnn — не-RU WARP через SOCKS5-egress (19.09-8).** **WARP egress country — CONNECTION-level, не account-level и не endpoint-level** (уточнение формулировки в `transport/wg/endpoints.go`): регистрация через не-RU SOCKS5 + прямое подключение → `loc=RU`; но **проведение MASQUE-H2 (TCP) через не-RU SOCKS5** → CF видит IP прокси → `loc=GB colo=LHR warp=on` (поле, env `B4_WARP_SOCKS5`, `src/warpservice/socks.go`). Значит страны WARP можно выбирать прокси-выходом. Ограничение: SOCKS5=TCP → только H2-карьер (UDP/QUIC не идёт; для `.40` `warp-tunnel` и так `upstream.udp=false`). Productionization — `b4x-w4c` (конфиг `system.warp.socks5` + routing set→`kind=masque` + live-gate `loc≠RU`). Doс `.ag/research/nonru-warp-socks5-2026-09-19.md`.

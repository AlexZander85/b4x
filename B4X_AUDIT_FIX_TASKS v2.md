...
| FB-07 | в работе (остался внешний гейт) | Beads b4x-070 (06.08→08.08): фазы A–D-1 закрыты (commits c266527c+39fdb9cf, a6332d47, 97146723): конфиг-поля `legacy_watchdog_direct_apply` (default false, §59) и `legacy_watchdog_api` (default true, §57.1) + startup warning + счётчик `monitor_legacy_watchdog_direct_apply_total`; MonitorAssessment + ShadowParityTracker; 410 Gone на legacy mutating `/api/watchdog/*` + read-only status alias из Monitoring-проекции (§58 с escalating-маппингом) + API migration tests; фаза D: IV-18 suite и FT-MON-A..J gates (hard_gates_ftmon_test.go) PASS, mon-11/mon-12/mon-13 приведены к фактическому статусу; полный CI green (08.08: `go vet ./...` + `go test ./...` — все пакеты ok). Осталось (внешние, не программные): MON_PRODUCTION_READY field evidence — real-router/Android field runs, multi-WAN/restart/reboot/fault-injection, privacy audit; legacy route removal после ≥1 compatible minor. Уже готово из FB-28: src/monitor/* canonical model, monitoring.Runtime в main.go, /api/monitor/v1, 52 hard-gate producer'а |
| FB-08 | выполнена | — |
...
- **Проблема:** MON v1.0 не подключён; legacy `applyBatchResults` и direct apply остаются active source of truth; `/api/monitor/v1` отсутствует; Monitoring gates/suites отсутствуют; конфиг-поле `legacy_watchdog_direct_apply` (MON addendum §77, default `false`, при `true` — startup warning) не реализовано; отчёты `docs/reports/mon-11-compatibility-cutover.md` и `docs/reports/mon-12-field-validation.md` содержат ложное утверждение «Direct legacy Watchdog apply remains disabled on the production-safe compatibility path» (независимый аудит B4X-AUDIT-0009): `applyBatchResults` активен (`watchdog/applier.go:18`, `watchdog_heal.go:111`).
  ...
  **Прогресс (06.08, Beads b4x-070):** ФБ-28 уже закрыл часть: `applyBatchResults` удалён, canonical model (`src/monitor/*`), `monitoring.Runtime` стартует в `main.go`, `/api/monitor/v1` (read-only), 52 §84-92 producer'а, `LegacyWatchdogAdapter`. В этой сессии: конфиг-поле `legacy_watchdog_direct_apply` (default `false`) в `WatchdogConfig` + startup warning + инкремент `monitor_legacy_watchdog_direct_apply_total` при `true` (блокирует production readiness, §59); тест дефолта; отчёты mon-11/mon-12 исправлены на фактический статус. Далее: MonitorAssessment + ShadowParityTracker (фаза A закрыта, commits c266527c+39fdb9cf). **Фаза C (core, commit a6332d47):** конфиг-поле `legacy_watchdog_api` (default `true`, §57.1) + event-driven cutover в `http/handler/watchdog.go` — при `false` все legacy mutating `/api/watchdog/*` (check/domains/domains/{domain}/enable/disable) отвечают **410 Gone** (стабильное миграционное сообщение), `GET /api/watchdog/status` — read-only alias поверх Monitoring-проекции (§58 vocabulary); API migration tests (`watchdog_cutover_test.go`, 9 проверок); mon-11/mon-12 обновлены; startup warning при cutover; полный CI green. **Фазы C-остаток + D закрыты (commit 97146723):** escalating-маппинг §58 в status alias, IV-18 suite и FT-MON-A..J gates (hard_gates_ftmon_test.go) PASS, mon-13 readiness matrix (шесть S57.1 prerequisites READY; `MON_PRODUCTION_READY` не claim'ится — требует внешних field evidence). Осталось — вне программной части: MON_PRODUCTION_READY field evidence (real-router/Android field runs, multi-WAN/restart/reboot/fault-injection, privacy audit — нужен hardware access) и removal legacy-маршрутов после ≥1 compatible minor (политика).

...
The legacy HTTP surface is governed by the cutover flag `legacy_watchdog_api` (MON addendum v1.0 §57.1, default `true`): with `false` the mutating legacy endpoints answer 410 Gone and `GET /api/watchdog/status` becomes a read-only alias over the Monitoring projection (§58), so no legacy state can be mistaken for an authorization source after cutover. API-migration tests: `go test ./http/handler -run 'WatchdogCutover|PreCutover'` (PASS).

...
type Runtime struct {
        ...
        // outcomes is the per-ADDRESS attempt memory (Nova 1.32.3): it feeds
        // the candidate ORDER — proven nodes lead, recently failed ones sink
        // — so a dead node is not retried first wave after wave.
        outcomes *proton.OutcomeMemory
        now      func() time.Time
...
func Build(cfg *config.Config, opts Options) (*Runtime, error) {
        ...
        r := &Runtime{
                ...
                outcomes: proton.NewOutcomeMemory(opts.Now),
                now:      opts.Now,
...
func (r *Runtime) ensureSession(ctx context.Context) error {
        ...
        // Nova 1.32.3 connectOrder: the outcome memory re-orders the pairs
        // (proven addresses lead with one pair, fresh ones breadth-first,
        // failed ones trail oldest-failure-first) and the top nodes of each
        // country carry port fallbacks — a filtered port no longer costs the
        // whole address.
        cands := queue.CandidatesOrdered(loc, r.outcomes)
        if len(cands) == 0 {
        ...
        seek, serr := twg.NewSeeker(twg.SeekerConfig{
                ...
                OnEvent: func(rec twg.AttemptRecord) {
                        ...
                        // Nova 1.32.3: every failed attempt marks the ADDRESS (ports
                        // rotate on refresh; the address outlives them). Winners are
                        // recorded at establishment — a handshake alone proves less.
                        if rec.Outcome != "winner" {
                                r.outcomes.RecordFailure(rec.Endpoint.Addr().String())
                        }
                },
        ...
        s, err = twg.NewSession(twg.SessionConfig{
                ...
                Callbacks: twg.SessionCallbacks{
                        OnEstablished: func() {
                                ...
                                r.mu.Unlock()
                                // Nova 1.32.3 outcome memory: establishment is
                                // the data-plane confirmation that marks the
                                // ADDRESS proven — it takes the head of the
                                // next seek order (freshest first).
                                r.outcomes.RecordSuccess(node.EntryIP)

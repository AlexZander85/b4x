...
func (api *API) handleStartDiscovery(w http.ResponseWriter, r *http.Request) {
	...
	if req.AdaptiveSynthesis != nil && req.AdaptiveSynthesis.Allowed {
		...
		in, runners, err := api.buildAutomaticSynthesis(*req.Scope, req.AdaptiveSynthesis, cfg)
		if err != nil {
			writeJsonError(w, http.StatusConflict, err.Error())
			return
		}
		result, err := discovery.RunAutomaticSynthesis(r.Context(), cfg, in, runners.baseline, runners.synthesized, runners.behavioral)
		if err != nil {
			writeJsonError(w, http.StatusConflict, err.Error())
			return
		}
		sendResponse(w, map[string]any{
			...
			"candidates_generated":   result.Result.Generated,
			"candidates_tested":      len(result.Result.TestedCandidates),
			"best_candidate_id":      result.Result.BestCandidateID,
			"stop_reason":            result.Result.StopReason,
			"behavioral_evidence_id": result.Evidence.EvidenceID,
		})
		return
	}

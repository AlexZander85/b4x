...
func TestWatchdogVersionMismatchSignature(t *testing.T) {
        ...
        wd := NewWatchdog(WatchdogConfig{
                ...
                Tick:   time.Second,
                // Byte floors lowered to 1: this test pins the version-mismatch
                // TRIGGER in the regime where every byte of rx counts as life. With
                // the DEFAULT floors (256 B) the sub-floor drip below is diagnosed
                // earlier and more accurately as ClassStallRX by the rx-idle
                // trigger (a completed handshake means the params are NOT wrong —
                // see TestWatchdogDripFiresStallRXWithDefaultFloors).
                MinRXGrowth: 1,
                MinTXGrowth: 1,
        ...
        for i := 0; i <= 130; i++ { // 131 s of samples
                ...
                // Monotonic sub-KB drip (1 B/s): with floors=1 every sample
                // re-arms the rx anchor, so trigger 1 stays quiet and the
                // full-window signature is what fires. (The old i%3 fixture
                // oscillated the counter, which real byte counters never do.)
                rx := uint64(i)
...
// TestWatchdogDripFiresStallRXWithDefaultFloors pins the NEW partition of
// the low-rx drip (handshake/keepalive bytes only, real tx flowing): with
// the default 256 B floors the drip no longer re-arms the rx-idle anchor,
// so the verdict is ClassStallRX within ~RXIdle after the last anchor —
// seconds, not the 120 s version-mismatch window (Nova 1.31.1: "обрыв
// длится секунды, а не минуты"). A completed handshake proves the params
// match; the data plane is what died.
func TestWatchdogDripFiresStallRXWithDefaultFloors(t *testing.T) {
        now := time.Unix(0, 0)
        wd := NewWatchdog(WatchdogConfig{
                RXIdle: 10 * time.Second,
                Window: 120 * time.Second,
        ...
        t0 := now
        // Baseline anchors at t=0 with a healthy exchange.
        wd.Feed(CounterSample{Time: t0, RxBytes: 1000, TxBytes: 1000})
        for i := 1; i <= 16; i++ {
                now = t0.Add(time.Duration(i) * time.Second)
                tx := 1000 + uint64(i)*2000 // real outbound traffic
                rx := uint64(1000)
                if i%5 == 0 {
                        rx += 92 // a handshake/rekey reply every ~5 s — sub-floor drip
                }
                wd.Feed(CounterSample{Time: now, RxBytes: rx, TxBytes: tx})
                if wd.Fired() {
                        break
        ...
        if !wd.Fired() || fired == nil {
                t.Fatal("drip signature never fired with default floors")
        }
        if fired.Class != ClassStallRX {
                t.Fatalf("class=%s want wg-stall-rx (drip is a dead data plane, not a param mismatch)", fired.Class)
        }
}
        ...
        for _, jitter := range shifts {
                ...
                t.Run("jitter="+jitter.String(), func(t *testing.T) {
                        ...
                        wd := NewWatchdog(WatchdogConfig{
                                ...
                                Window:      120 * time.Second,
                                Tick:        time.Second,
                                MinRXGrowth: 1, // floors off: pin the trigger-2 mechanics
                                MinTXGrowth: 1,
                                Now:         func() time.Time { return now },
                        })
                        var fired *Failure
                        ...
                        for i := 0; i <= 140; i++ { // 141 s of samples
                                ...
                                now = stamp
                                tx := uint64(i) * 200 // ~200 B/s => crosses 4096 at ~21 s
                                rx := uint64(i)       // monotonic sub-KB drip (floors=1)
                                wd.Feed(CounterSample{Time: now, RxBytes: rx, TxBytes: tx})
                                if wd.Fired() {
                                        break
                                }

...
// PollActiveQuota re-mints the ACTIVE account's proxy pass so the fresh
// X-Quota-* numbers flow into the rotation triggers (design Ч.I §3: 15-min
// quota polling; review F7). Double benefit: the pass is re-issued earlier
// than the 2-min renewal lead. Exhaustion (429) and auth rejection
// (401/403) record the account state exactly like the fetch path; nil,nil
// means "nothing to report" (no active seat / no access token / state
// recorded), not success.
func (p *Pool) PollActiveQuota(ctx context.Context) (*ProxyPassInfo, error) {
        p.mu.Lock()
        if p.active < 0 {
                p.mu.Unlock()
                return nil, nil
        ...
        a := p.accounts[p.active]
        p.mu.Unlock()
        access := p.accessTokenOf(a)
        if access == "" {
                return nil, nil
        }

        pass, err := p.guardian.FetchProxyPass(ctx, access)
        if err != nil {
                var qe *QuotaError
                if errors.As(err, &qe) {
                        reset := time.Time{}
                ...
                return nil, err // pin mismatch / challenge / transport: fail-closed
        }

        p.mu.Lock()
        a.pass = pass
        left := parseQuotaInt(pass.QuotaLeft, a.quotaLeft)
        max := parseQuotaInt(pass.QuotaMax, a.quotaMax)
        a.quotaLeft, a.quotaMax = left, max
        if reset := parseResetTime(pass.QuotaReset); !reset.IsZero() {
                a.resetAt = reset
        }
        if !a.warned && max > 0 && left >= 0 && left*100 < max*int64(p.cfg.RotateThresholdPct) {
                a.warned = true
                p.emit(PoolEvent{Type: EvQuotaWarning, Label: labelOf(a),
                        Detail: strconv.FormatInt(left, 10) + "/" + strconv.FormatInt(max, 10)})
        }
...
func (p *Pool) Status() PoolStatus {
        p.mu.Lock()
        defer p.mu.Unlock()
        return p.statusLocked(p.cfg.Now())
}

// StatusTry snapshots the pool WITHOUT blocking: events are emitted while
// p.mu is held (setActiveIndex/swapTo/refreshFailed/...), and the FX5
// events hook re-enters the pool for the metrics gauges — a synchronous
// Status() there self-deadlocks (latent FX5 bug, caught by the F3 live-path
// test). Callers that may run on an event emission use StatusTry and skip
// on false; supervision ticks refresh the gauges regardless.
func (p *Pool) StatusTry() (PoolStatus, bool) {
        if !p.mu.TryLock() {
                return PoolStatus{}, false
        }
        defer p.mu.Unlock()
        return p.statusLocked(p.cfg.Now()), true
}

func (p *Pool) statusLocked(now time.Time) PoolStatus {
        st := PoolStatus{}
...
// ActiveBearer returns the RAW proxy pass for the serving session plus
// whether an active account exists at all. The carriers add the "Bearer "
// scheme themselves when stamping Proxy-Authorization (h2tunnel/h3tunnel) —
// returning the header value here produced "Bearer Bearer <token>" on the
// live wire (found during the F3 fix).
func (p *Pool) ActiveBearer() (string, bool) {
        ...
        return p.accounts[p.active].pass.RawToken, true
}

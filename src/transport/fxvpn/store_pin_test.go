...
// TestPinPendingNotPersistedBeforeProof pins review F5: an UNSEEDED host's
// first contact records the leaf only as an in-memory PENDING candidate —
// nothing is persisted, and the committed map stays empty until a parseable
// 2xx rode the channel (ControlPlane.Do -> CommitPending). With the old
// naive TOFU a first-contact MITM froze its key into pins.json forever.
func TestPinPendingNotPersistedBeforeProof(t *testing.T) {
        dir := t.TempDir()
        pinPath := filepath.Join(dir, "pins.json")
        ps, err := LoadPinStore(pinPath)
        if err != nil {
                t.Fatalf("load: %v", err)
        }

        srv, _ := tlsStand(t) // "attacker"/bootstrap leaf for an unseeded host
        host := "bootstrap.example"
        if err := ps.Verify(host, []*x509.Certificate{srv.Certificate()}); err != nil {
                t.Fatalf("verify (pending path): %v", err)
        }
        if _, ok := ps.Snapshot()[host]; ok {
                t.Fatal("pending candidate must NOT appear in the committed map")
        }
        if _, ok := ps.PendingSnapshot()[host]; !ok {
                t.Fatal("pending candidate missing from the pending map")
        }
        if _, statErr := os.Stat(pinPath); !errors.Is(statErr, fs.ErrNotExist) {
                t.Fatalf("store must not be created before proof: %v", statErr)
        }

        // The proof arrives: commit promotes and persists.
        recorded, cerr := ps.CommitPending(host)
        if !recorded || cerr != nil {
                t.Fatalf("commit: recorded=%v err=%v", recorded, cerr)
        }
        reloaded, err := LoadPinStore(pinPath)
        if err != nil {
                t.Fatalf("reload: %v", err)
        }
        if _, ok := reloaded.Snapshot()[host]; !ok {
                t.Fatalf("committed pin missing after proof: %+v", reloaded.Snapshot())
        }
}

// TestPinSeededHostDivergenceFailsClosed pins review F5b: a host with a
// baked seed presenting a FOREIGN key (a first-contact MITM, a rotated CDN
// key, or a stale seed) fails closed with the explicit seed-divergence
// error — it is never silently auto-committed.
func TestPinSeededHostDivergenceFailsClosed(t *testing.T) {
        ps, err := LoadPinStore(filepath.Join(t.TempDir(), "p.json"))
        if err != nil {
                t.Fatal(err)
        }
        srv, _ := tlsStand(t) // attacker leaf != baked seed
        for host := range seedPins {
                err := ps.Verify(host, []*x509.Certificate{srv.Certificate()})
                if !errors.Is(err, ErrPinMismatch) {
                        t.Fatalf("seeded host %s must fail closed, got %v", host, err)
                }
                if !strings.Contains(err.Error(), "seed divergence") {
                        t.Fatalf("seed divergence must be explicit: %v", err)
                }
        }
        if _, ok := ps.Snapshot()["vpn.mozilla.org"]; ok {
                t.Fatal("seed divergence must never commit")
        }
}

// TestPinCommitAfterSuccessfulExchange wires F5 end to end: the FIRST
// successful Guardian exchange through a pinned TLS stand leaves a
// committed, persisted pin (the 2xx proof path), and a different key on the
// same store mismatches afterwards.
func TestPinCommitAfterSuccessfulExchange(t *testing.T) {
        dir := t.TempDir()
        pinPath := filepath.Join(dir, "pins.json")
        ...
        cp.EP.Guardian = srvA.URL
        if _, ok := cp.Pins.Snapshot()[hostKey]; ok {
                t.Fatal("no pin may exist before the first exchange")
        }

        if _, err := (&Guardian{CP: cp}).FetchUserInfo(t.Context(), "tok"); err != nil {
                t.Fatalf("first contact through TLS+pin: %v", err)
        }
        ...
        if _, ok := pins.Snapshot()[hostKey]; !ok {
                t.Fatalf("2xx proof must commit the pin for %s: %+v", hostKey, pins.Snapshot())
        }
}

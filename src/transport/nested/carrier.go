...
// (post-connect facts, not config declarations). (PATCH-17, M-4 / B-N3.)
...
// edgeCollision reports a post-connect edge collision between the two
// composed layers. Config-time rejects (PairConfig.Validate) compare the
// DECLARED IPs; this check compares what the layers actually reached —
// the nesting-into-itself loop the checklist B-N3 requires to be detected.
func edgeCollision(outer, inner edgeWitness) bool {
	if outer.ip != "" && outer.ip == inner.ip {
		return true
	}
	if outer.colo != "" && outer.colo == inner.colo {
		return true
	}
	return false
}
...
//
// PATCH-15 (M-2), owner verdict B: the dead AttemptV6 flag is REMOVED.
// Dual-family pinning (v4 mandatory + v6 warn) is consciously not
// implemented: addendum §46 keeps the tunnel scope v4-only until a separate
// validation opens IPv6, and unvalidated paths are never shipped. Restore
// the flag together with a two-family Setup when that scope opens.

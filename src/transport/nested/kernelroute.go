...
func (c *KernelRouteCarrier) Setup(ctx context.Context) error {
	...
	if err := c.pinFamily(ctx, ep); err != nil {
		if isMandatoryFamily(ep.Addr(), c.policy) {
			...
			c.emit(Event{Class: ClassCarrierRouteLost,
...
func (c *KernelRouteCarrier) pinFamily(ctx context.Context, ep netip.AddrPort) error {
	...
	out, err := c.cfg.Runner(ctx, fam, "route", "replace", dst.String()+"/"+plen, "dev", c.cfg.Device)
	if err != nil {
		// add->replace fallback discipline: clear a stale conflicting entry
		// once, then retry the replace (EEXIST-class recovery).
		if _, delErr := c.cfg.Runner(ctx, fam, "route", "del", dst.String()+"/"+plen); delErr == nil {
			var err2 error
			if _, err2 = c.cfg.Runner(ctx, fam, "route", "replace", dst.String()+"/"+plen, "dev", c.cfg.Device); err2 == nil {
				err = nil
			} else {
				err = err2
			}

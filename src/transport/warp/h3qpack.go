// Minimal QPACK (RFC 9204) codec for the H3 MASQUE transport — path B of the
// E-H3 design (§2): a hand-written HTTP/3 layer over raw quic-go streams,
// mirroring the warp-socks minimal-H3 proof against production Cloudflare.
//
// Scope (deliberate, design §2 "не нужны" list):
...
var ErrQPACKDynamic = errors.New("transportwarp: qpack dynamic/post-base reference in field section")
...
// EncodeConnectFieldSection builds the QPACK field section of the MASQUE
// extended-CONNECT request: :method CONNECT (indexed static), :authority
// (literal, static name reference — the warp-socks 0x50 pattern), then each
// extra header as a never-indexed literal-with-literal-name line (§4.5.6,
// N=1). No Host header is ever emitted (design §4: IP authority only,
// host рядом с IP-authority = H3_MESSAGE_ERROR 270).
...
func DecodeFieldSection(b []byte) ([][2]string, error) {
	...
	if sign != 0 {
		...
		return nil, errors.New("transportwarp: qpack negative base in field section")
	}
...
func (c *qpackReader) finishInt(acc uint64) (uint64, error) {
	...
	for {
		...
		shift += 7
		if shift > 56 {
			return 0, errors.New("transportwarp: qpack integer overflow")
		}
	}
}
...
func staticLookup(idx uint64) (qpackEntry, error) {
	if idx >= uint64(len(qpackStaticTable)) {
		return qpackEntry{}, fmt.Errorf("transportwarp: qpack static index %d out of range", idx)
	}
...
func (c *qpackReader) readString(prefixBits int) (string, error) {
	...
	if n > qpackMaxStringLength {
		return "", fmt.Errorf("transportwarp: qpack string too long (%d)", n)
	}
	raw := make([]byte, n)
	for i := range raw {
	...
	if err != nil {
		return "", fmt.Errorf("transportwarp: qpack huffman decode: %w", err)
	}
	return s, nil
}
...
func (c *qpackReader) readBits(n uint) (uint64, error) {
	if c.pos+int(n) > len(c.buf)*8 {
		return 0, errors.New("transportwarp: qpack unexpected end of field section")
	}
	var v uint64
	for i := uint(0); i < n; i++ {
		byteIdx := c.pos / 8

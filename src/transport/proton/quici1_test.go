...
func TestQuicInitialDecryptsPerRFC9001(t *testing.T) {
	...
	// Walk to the extensions tail: legacy_version(2)+random(32)+3 empties.
	extBlob := ch[42:]
	// extensions_len(2) + ext_type(2)=0000 + ext_len(2) + list_len(2) +
	// name_type(1)=0 + host_len(2) + host.
	if !bytes.Equal(extBlob[2:4], []byte{0x00, 0x00}) {
		t.Fatalf("extension type = %x, want server_name", extBlob[2:4])
	}
	hostLen := int(binary.BigEndian.Uint16(extBlob[9:11]))
	gotSNI := string(extBlob[11 : 11+hostLen])
	if gotSNI != sni {

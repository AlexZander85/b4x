...
func newGuardianStand() *guardianStand { return newGuardianStandTTL(time.Hour) }

// newGuardianStandTTL mints proxy passes with the given TTL (a short TTL puts
// the active pass inside the pool's renewal lead).
func newGuardianStandTTL(ttl time.Duration) *guardianStand {
	if ttl <= 0 {
		ttl = time.Hour
	}
	g := &guardianStand{quotaLeft: "500", quotaMax: "1000", passTTL: ttl}
	mux := http.NewServeMux()
	mux.HandleFunc("/api/v1/fpn/token", func(w http.ResponseWriter, r *http.Request) {
		g.mu.Lock()
		left, max, ttl := g.quotaLeft, g.quotaMax, g.passTTL
		g.mu.Unlock()
		if left != "" {
		...
		w.WriteHeader(http.StatusOK)
		_ = json.NewEncoder(w).Encode(map[string]string{"token": testJWTWithTTL(ttl)})
	})
...
func testJWT() string { return testJWTWithTTL(time.Hour) }

// testJWTWithTTL mints a pass with the given lifetime (short TTL => the pool
// sees the active pass inside its renewal lead).
func testJWTWithTTL(ttl time.Duration) string {
	payload := fmt.Sprintf(`{"sub":"test-sub","aud":"fpn","iss":"guardian","iat":%d,"nbf":%d,"exp":%d}`,
		time.Now().Unix(), time.Now().Unix()-1, time.Now().Add(ttl).Unix())
...
func newLiveFixture(t *testing.T, rotatePct int) *liveFixture {
	return newLiveFixtureTTL(t, rotatePct, time.Hour)
}

// newLiveFixtureTTL builds the fixture with a custom proxy-pass lifetime
// (short TTL exercises the renewal path).
func newLiveFixtureTTL(t *testing.T, rotatePct int, passTTL time.Duration) *liveFixture {
	t.Helper()
	...
	fx := &liveFixture{fxa: newFxAStand(), guardian: newGuardianStandTTL(passTTL)}

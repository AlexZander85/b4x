...
// CoalesceInitial prepends a padding-only dummy Initial to a real client Initial
// inside one datagram. The dummy is derived from a flipped-DCID (so neither the
// server nor a DPI can decrypt it under the header DCID it carries), the server
// discards it as undecryptable and processes the second packet, while a DPI that
// inspects only the first packet of a datagram stops on the dummy and never reaches
// the handshake. The real Initial is left byte-for-byte unchanged.

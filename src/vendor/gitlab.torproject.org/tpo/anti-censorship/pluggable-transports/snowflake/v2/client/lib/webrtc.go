...
// NetWrapper, when set, wraps the network pion gets through
// SettingEngine.SetNet. Installed once by the embedding process.
...
func (c *WebRTCPeer) preparePeerConnection(
	...
	// b4x fork (Nova snowflake.patch port, design E-TOR §10): NetWrapper
	// lets the embedding process replace the network pion receives. The
	// ICE/DTLS sockets pion creates itself would otherwise leave outside
	// the embedding process's egress policy (marking / carrier
	// composition) — there is no other interception point, the
	// SettingEngine is assembled right here.
	if NetWrapper != nil {

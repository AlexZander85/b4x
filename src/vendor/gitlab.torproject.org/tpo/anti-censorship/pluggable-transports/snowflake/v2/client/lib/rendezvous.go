...
//
// b4x fork (Nova snowflake.patch port, design E-TOR §10): BrokerDialContext,
// when set, replaces the rendezvous transport's dialing — the broker
// exchange rides ordinary sockets and inside an embedding VPN those
// sockets must go through the embedding process's egress dialer.

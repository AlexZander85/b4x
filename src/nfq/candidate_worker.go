...
func (w *Worker) StartCandidate() error {
        ...
        w.q = q
        // Netlink read buffer — the same #296 lesson as the main queue: a
        // candidate queue that overflows reads as "no packets observed",
        // skewing discovery verdicts. Keep the drain wide.
        if err := q.Con.SetReadBuffer(4 << 20); err != nil {
                log.Warnf("candidate nfqueue read buffer sizing failed (queue=%d): %v", w.qnum, err)
        }

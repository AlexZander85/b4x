...
func (w *Worker) Start() error {
        ...
        // Netlink read buffer (upstream b4 lesson, issue #296): the kernel
        // silently drops queued packets when the receive buffer overflows —
        // on a busy router the default (a few hundred KB) is not enough and
        // manifests as multi-second DNS/classification stalls without any
        // error anywhere. 4 MB keeps the queue draining under burst load.
        if err := q.Con.SetReadBuffer(4 << 20); err != nil {
                log.Warnf("nfqueue read buffer sizing failed (queue=%d): %v", w.qnum, err)
        }

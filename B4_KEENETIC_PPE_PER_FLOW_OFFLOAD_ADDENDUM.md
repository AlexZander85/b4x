...
### 3.2.1. Provenance и авто-интеграция (FB-21)

Любое переключение toggle в UI/API обязано устанавливать признак явного
выбора оператора (`capture.offload_policy_user_chosen = true`), который
длительно сохраняется в конфигурации. Значение с установленным признаком
никогда не переопределяется логикой запуска продукта.

Без признака (новая установка, `detect`) продукт может один раз, при
старте, автоматически интегрировать per-flow исключение, но только если
выполнены ВСЕ условия:

- платформа Keenetic NDM подтверждена;
- SoC MediaTek/MT762;
- capability-probe per-flow exclusion = PASS;
- transactional prepare успешен;
- pre-commit visibility self-test = PASS;
- durable persist конфигурации успешен.

При любом FAIL/UNKNOWN результат тот же самый: политика остаётся/возвращается
в `detect`, ноль утёкших правил. `disable-global` и `exclude`, выбранные
пользователем, автоматически не включаются и не выключаются никогда.
...
Авто-интеграция (FB-21) не является действием для оператора: это внутренний
start-time сценарий с обязательным pre-commit self-test. UI должен показывать
`detect` (или фактическое состояние правил) до тех пор, пока авто-сценарий не
завершился успешно и конфигурация не зафиксирована; после успеха состояние
совпадает со штатным `exclude`.

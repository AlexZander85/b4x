...
> **ПОЛЕВАЯ ПОПРАВКА 2026-09-21** (реальный прогон на живом Keenetic, отчёт
> `.ag/research/utls-field-report.md`). Ожидания ниже уточнены по фактам провода —
> не считать отклонениями:
> - **Доступность:** весь `fastly-masque` эдж — `23.235.42.0/24`, TCP `:2499` из РФ
>   мгновенно недостижим (pitfall 29). Поэтому **Step 1 (H2) полем непроверяем** (CH не
>   выходит), а **Step 2 (H3) проверяем только по отпечатку** — сессия встаёт лишь во
>   вложении через носитель (AWG-WARP), где внутренний CH шифрован и JA4 не виден.
> - **Ваниль ≠ ML-KEM:** в этом билде кривые пинованы (b4x-nxx), ванильные группы =
>   `0x0017,0x0018`, без `0x4588`. Дельта Go↔Firefox — **порядок/счётчики расширений**,
>   а не отсутствие ML-KEM.
> - **Firefox-uTLS QUIC без GREASE** (ожидание GREASE в Step 1 не подтверждается).
> - **Приманка** — настоящий RFC-9001 Initial с шифрованным CRYPTO и **пустым DCID**
>   (ключи статичны); белый SNI виден только расшифровщику, tshark не декодит. Приманка
>   уходит с **нового** сокета (другой src-port), не из 5-tuple реального потока — «тот же
>   5-tuple» технически недостижим без per-packet TTL (cmsg).
> - **Удержание (Step 6):** data-plane умирает через ~10 мин из-за истечения proxy bearer
>   (`HTTP 401 token expired`); исправлено in-place renewal'ом (`b4x-0rw8`).
> - **Step 8:** детектор блока на реальном `EHOSTUNREACH` срабатывает после фикса
>   (`b4x-3pmi`); проверять с `ladder nested:false` и `nest_on_port_block:true`.

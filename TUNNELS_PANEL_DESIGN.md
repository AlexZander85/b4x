...
- **Жёсткая QUIC-политика для TCP-only туннелей: `routing.quic: "block"`.** Для сета с `routing.mode=tunnel` и TCP-only карьером (`fxvpn`/`opera`/`tor`/…) в `chainPre` ставится **drop UDP для ipset целей сета** → QUIC не может уйти мимо туннеля, клиент падает на TCP, который и едет в туннель. `""`/`"auto"` — прежнее поведение (UDP не несётся, правило не ставится). Для UDP-карьеров (`warp`/`proton`) игнорируется (`upstream.udp=true` реально туннелирует UDP). Поле `RoutingConfig.Quic` + валидация (`auto|block`). Полевая валидация — после окна.
